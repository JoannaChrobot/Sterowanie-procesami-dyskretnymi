...
namespace dzisiaj_SPD_jutro_listonosz_w_DPD
{
    internal class PD
    {
        ...
        public void PD_Algorithm(List<Task> problemTasks, Machine machine1, Machine machine2)
        {
 
            int nRows = problemTasks.Count + 1;
            int sum_proccess = 0;

            foreach (Task task in problemTasks) // wyliczamy ilosc kolumn
            {
            ...
            for (int row = 0; row < nRows; row++) // inicjalizujemy
            {
                T[row, 0] = 1;
            ...
            for (int row = 1; row < nRows; row++)
            {
                for (int col = 0; col < nCols; col++)
                {
                    if (T[row - 1, col] == 1)
                    {
                        T[row, col] = 1; // przepisujemy jedynke z rzedu wyzej
                        if (col + problemTasks[row - 1].processingTime < nCols) // i dodajme jedynke o processing time
                        {
